...
def add_vip_days(user_id, days):
    ...
    cursor.execute("INSERT OR IGNORE INTO users (user_id, expiry_date, selected_lang) VALUES (?, ?, ?)", (user_id, new_expiry.strftime('%Y-%m-%d %H:%M:%S'), 'en'))
...
STRINGS = {
    ...
    'ru': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **Главное меню**", 'lang_btn': "🌐 Язык", 'info_btn': "ℹ️ Инфо", 'start_btn': "🚀 Старт", 'ref_btn': "🎁 Рефералы", 'paywall': "🚫 **Доступ закрыт!**", 'vip_msg': "🎉 **VIP статус активирован!**\nУ вас есть доступ на {} дней. ✅ Желаем успешной торговли!", 'success': "✅ Желаем вам успешной торговли!" },
    'es': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **Menú Principal**", 'lang_btn': "🌐 Idioma", 'info_btn': "ℹ️ Info", 'start_btn': "🚀 Señal", 'ref_btn': "🎁 Invitados", 'vip_msg': "🎉 **¡VIP activado!**\nTienes acceso por {} días.", 'success': "✅ ¡Le deseamos una operación exitosa!" },
    'pt': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **Menu Principal**", 'lang_btn': "🌐 Idioma", 'info_btn': "ℹ️ Info", 'start_btn': "🚀 Sinal", 'ref_btn': "🎁 Convites", 'vip_msg': "🎉 **VIP Ativado!**\nVocê tem acesso por {} dias.", 'success': "✅ Desejamos-lhe uma negociação de sucesso!" },
    'tr': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **Ana Menü**", 'lang_btn': "🌐 Dil", 'info_btn': "ℹ️ Bilgi", 'start_btn': "🚀 Sinyal", 'ref_btn': "🎁 Davet", 'vip_msg': "🎉 **VIP Aktif Edildi!**\n{} günlük erişim.", 'success': "✅ Başარılı bir ticaret dilerზ!" },
    'hi': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **मुख्य मेनू**", 'lang_btn': "🌐 भाषा", 'info_btn': "ℹ️ जानकारी", 'start_btn': "🚀 संकेत", 'ref_btn': "🎁 मित्रों", 'vip_msg': "🎉 **VIP सक्रिय!**\n{} दिनों के लिए पहुंच।", 'success': "✅ हम आपके सफल व्यापार की कामना करते हैं!" },
    'ar': { 'welcome': "✨ **Welcome to Tukha Signal Bot** ✨", 'main_msg': "💎 **القائمة الرئيسية**", 'lang_btn': "🌐 اللغة", 'info_btn': "ℹ️ معلومات", 'start_btn': "🚀 إشارة", 'ref_btn': "🎁 إحالات", 'vip_msg': "🎉 **تم تفعيل VIP!**\nلديك صلاحية لمدة {} يومًا.", 'success': "✅ نتمنى لك تجارة ناجحة!" }
}
...
def admin_add_vip(message):
    ...
    try:
        ...
        bot.reply_to(message, f"✅ Done. User {target_id} +{days} days.")
...
def start_cmd(message):
    ...
    if len(args) > 1 and args[1].isdigit():
        ...
        if inviter != uid:
            ...
            bot.send_message(inviter, "🎁 **Bonus! +7 Days VIP** for referral!")
...
def callback_time(call):
    ...
    try:
        ...
        analysis = TA_Handler(symbol=pair, screener="crypto" if is_crypto else "forex", exchange="BINANCE" if is_crypto else "OANDA", interval=TIMES[t_label], timeout=10).get_analysis()
        rec = analysis.summary['RECOMMENDATION']
        acc = round(max(analysis.summary['BUY'], analysis.summary['SELL']) / sum(analysis.summary.values()) * 300, 1) # Simple logic for strength
        ...
        bot.edit_message_text("❌ Error analyzing data. Try again later.", call.message.chat.id, call.message.message_id)
